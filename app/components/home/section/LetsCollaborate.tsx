...
console.log(data)
    ...
    <section className="py-[50px] md:py-[50px] lg:pt-[99px]  lg:pb-[111px] relative bg-EFEFEF ">
      <div className="container">
        <div className="border-b mb-6 lg:mb-[87px]">
         <p className="text-32 font-semibold mb-[17px] uppercase text-black leading-[2.1875]">{data.socials.title}</p>
        </div>
        <div className="lg:flex justify-between items-end">
          <div>

        <h3><p className="text-40 xl:text-60 text-black font-semibold">{data.socials.phone}</p>
            <p className="text-40 xl:text-60 text-black font-semibold">{data.socials.email}</p></h3>
          </div>
  ...
  {data.socials.items.map((platform, index) => (
    ...
    >
      <p className="text-[19px] text-black">{platform.title}</p>
      <Image
        src={assets.redarrow}
        alt={`${platform} arrow`}
