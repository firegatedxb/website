...
>
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-[8px]">
    {systemss?.systems?.map((item, index) => (
      <div key={index}>
        <div className="relative group rounded-[20px] overflow-hidden h-full bg-EFEFEF hover:bg-secondary hover:text-white flex flex-col transition-all duration-500">
        ...
        viewport={{ once: true, amount: 0.3 }}
          >
            <Image
              ...
              width={399}
              height={381}
              className="w-full object-cover group-hover:scale-105 transition-all duration-400"
            />
          </motion.div>

          <h3 className="text-32 font-medium leading-[1.3] text-black group-hover:text-white duration-400 transition-colors px-5 pt-5 lg:px[28px] lg:pt-8">
            {item.title}
          </h3>

          <div className="mt-auto px-5 py-5 lg:py-8 lg:px[28px]">
            <Link href={`systems/${item.slug}`}>
              <button className="flex py-[7px] px-[20px] items-center justify-between text-15 text-black bg-white group-hover:bg-primary group-hover:text-white font-medium rounded-[20px] space-x-5 text-xs leading-[1.87] uppercase w-full">
                <span>READ MORE</span>
