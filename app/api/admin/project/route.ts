...
export async function POST(req:NextRequest) {
    try {
        await connectDB();
        const {name,slug,client,sector,consultant,location,title,description,images,thumbnail,thumbnailAlt,coverPhoto,coverPhotoAlt,metaTitle,metaDescription,featuredProject} = await req.json();
        const project = await Project.create({name,slug,client,sector,consultant,location,title,description,images,thumbnail,thumbnailAlt,coverPhoto,coverPhotoAlt,metaTitle,metaDescription,featuredProject});
        if(project){
...
export async function PATCH(req:NextRequest) {
    try {
        ...
        const {name,slug,client,sector,consultant,location,title,description,images,thumbnail,thumbnailAlt,coverPhoto,coverPhotoAlt,metaTitle,metaDescription,featuredProject} = await req.json();
        const project = await Project.findByIdAndUpdate(id,{name,slug,client,sector,consultant,location,title,description,images,thumbnail,thumbnailAlt,coverPhoto,coverPhotoAlt,metaTitle,metaDescription,featuredProject});
        if(project){
